...
class AuditLogTest extends TestCase
{
    ...
    public function test_criacao_transacao_registra_auditoria(): void
    {
        ...
        $this->postJson('/api/financeiro/transactions', [
        ...
        $this->assertTrue(
            AuditLog::query()->where('action', 'financial_transaction.created')->exists(),
    ...
    public function test_atualizacao_transacao_registra_auditoria(): void
    {
        ...
        $this->putJson("/api/financeiro/transactions/{$transaction->id}", [
            'descricao' => 'ajuste auditoria',
        ])->assertOk();

        $this->assertTrue(
            AuditLog::query()->where('action', 'financial_transaction.updated')->where('auditable_id', $transaction->id)->exists(),
