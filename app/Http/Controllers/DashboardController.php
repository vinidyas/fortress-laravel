...
class DashboardController extends Controller
{
    ...
    private function buildFinancialTrend(Carbon $now): array
    {
        ...
        $transactions = FinancialTransaction::query()
            ->whereBetween('data_ocorrencia', [$start->toDateString(), $end->toDateString()])
            ->get(['data_ocorrencia', 'tipo', 'valor']);

        foreach ($transactions as $transaction) {
            if (! $transaction->data_ocorrencia) {
                continue;
            }

            $bucket = $transaction->data_ocorrencia->startOfMonth()->format('Y-m');
            ...
            } elseif ($transaction->tipo === 'debito') {
