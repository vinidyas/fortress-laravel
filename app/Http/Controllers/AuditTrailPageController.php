...
class AuditTrailPageController extends Controller
{
    ...
    public function index(Request $request): Response
    {
        ...
        $guardsQuery = AuditLog::query()
            ->whereNotNull('context->guard');

        $guardExpression = $this->jsonValueExpression($guardsQuery, 'context', 'guard');
        $guards = $guardsQuery
            ->selectRaw("DISTINCT {$guardExpression} AS guard")
            ->orderBy('guard')
            ->pluck('guard')
            ->filter()
            ->values();

        $originsQuery = AuditLog::query()
            ->whereNotNull('context->origin');

        $originExpression = $this->jsonValueExpression($originsQuery, 'context', 'origin');
        $origins = $originsQuery
            ->selectRaw("DISTINCT {$originExpression} AS origin")
            ->orderBy('origin')
            ->pluck('origin')
            ->filter()
            ->values();

        $methodsQuery = AuditLog::query()
            ->whereNotNull('context->http_method');

        $methodExpression = $this->jsonValueExpression($methodsQuery, 'context', 'http_method');
        $requestMethods = $methodsQuery
            ->selectRaw("DISTINCT {$methodExpression} AS method")
            ->orderBy('method')
            ->pluck('method')
            ->filter()
            ->values();
