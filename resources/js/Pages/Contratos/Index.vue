        ...
        <div class="mb-6 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <form @submit.prevent="applyFilters" class="grid gap-4 md:grid-cols-4">
                <div>
                    ...
                    <input v-model="filters.search" type="search" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" placeholder="Codigo" />
                </div>
                <div>
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700">Cidade do imovel</label>
                    <input v-model="filters.cidade" type="text" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700">Imovel ID</label>
                    <input v-model="filters.imovel_id" type="number" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700">Locador ID</label>
                    <input v-model="filters.locador_id" type="number" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700">Locatario ID</label>
                    <input v-model="filters.locatario_id" type="number" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700">Dia vencimento</label>
                    <input v-model="filters.dia_vencimento" type="number" min="1" max="28" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700">Vigencia em</label>
                    <input v-model="filters.vigencia_em" type="date" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" />
                </div>
                <div>
            ...
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Locador</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Locatario</th>
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Inicio / Fim</th>
                ...
                <tbody class="divide-y divide-slate-200 text-sm text-slate-700">
                    <tr v-if="loading">
                        <td colspan="7" class="px-4 py-6 text-center text-slate-500">Carregando contratos...</td>
                    </tr>
                    <tr v-else-if="!contratos.length">
                        <td colspan="7" class="px-4 py-6 text-center text-slate-500">Nenhum contrato encontrado.</td>
                    </tr>
                    <tr v-for="contrato in contratos" :key="contrato.id">
                        ...
                        <td class="px-4 py-3 text-right">
                            <Link :href="`/contratos/${contrato.id}`" class="text-sm font-semibold text-indigo-600 hover:text-indigo-500">Editar</Link>
                        </td>
                    </tr>
                </tbody>
            </table>
