...
async function loadContrato() {
  ...
  try {
    ...
    console.error(error);
    errorMessage.value = 'Não foi possível carregar o contrato.';
  } finally {
    loading.value = false;
  }
}
...
async function submit() {
  ...
  try {
    ...
    if (error?.response?.status === 422) {
      ...
      errorMessage.value = 'Não foi possível salvar o contrato.';
    }
  } finally {
    saving.value = false;
  }
...
<template>
  <AuthenticatedLayout :title="isEditing ? 'Editar contrato' : 'Novo contrato'">
    <div class="mb-6 flex items-center justify-between">
      ...
      <Link class="text-sm font-semibold text-indigo-600 hover:text-indigo-500" href="/contratos"
        >Voltar</Link
      >
    </div>
