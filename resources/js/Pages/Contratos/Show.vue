...
async function copyLinhaDigitavel(linha: Nullable<string>, faturaId: number) {
  if (!linha) return;

  if (!clipboardSupported) {
    faturasError.value = 'Copiar linha digitável não é suportado neste dispositivo.';
    return;
  }
  ...
  try {
    ...
    console.error(error);
    faturasError.value = 'Não foi possível copiar a linha digitável.';
  } finally {
                    ...
                    <table class="min-w-full divide-y divide-slate-800 text-sm">
                      <thead class="text-xs uppercase tracking-wide text-slate-400">
                        <tr>
                          ...
                          <th class="px-3 py-2 text-left font-semibold">Boleto</th>
                        </tr>
                      </thead>
                          ...
                          <td class="px-3 py-2">
                            <template v-if="fatura.boleto_atual">
                              <div class="space-y-1 text-xs text-slate-400">
                                ...
                                <span v-if="fatura.boleto_atual?.nosso_numero" class="block">
                                  Nosso nº {{ fatura.boleto_atual.nosso_numero }}
                                </span>
                                <span v-if="fatura.boleto_atual?.linha_digitavel" class="block text-slate-500">
                                  {{ formatLinhaDigitavel(fatura.boleto_atual.linha_digitavel) }}
                                </span>
                                <div class="flex flex-wrap items-center gap-2 text-[11px]">
                                  <button
                                    v-if="fatura.boleto_atual?.linha_digitavel && clipboardSupported"
                                    type="button"
                                    class="inline-flex items-center gap-1 rounded border border-slate-700 px-2 py-1 transition hover:border-indigo-500 hover:bg-indigo-500/20 hover:text-white disabled:cursor-not-allowed disabled:opacity-60"
                                    :disabled="copyingLinhaDigitavelId === fatura.id"
                                    @click="copyLinhaDigitavel(fatura.boleto_atual?.linha_digitavel ?? null, fatura.id)"
                                  >
                                    ...
                                    <span>{{ copyingLinhaDigitavelId === fatura.id ? 'Copiando…' : 'Copiar' }}</span>
                                  </button>
                                  <a
                                    ...
                                    class="inline-flex items-center gap-1 rounded border border-indigo-500/40 px-2 py-1 text-indigo-200 transition hover:border-indigo-400 hover:bg-indigo-500/20 hover:text-white"
                                  >
                                    <svg class="h-3 w-3" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
