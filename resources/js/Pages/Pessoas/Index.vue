...
async function fetchPessoas(page = 1) {
    ...
    try {
        ...
        console.error(error);
        errorMessage.value = 'Nao foi possivel carregar as pessoas.';
    } finally {
        loading.value = false;
    }
}
        ...
        <div class="mb-6 rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <form @submit.prevent="applyFilters" class="grid gap-4 md:grid-cols-5">
                <div class="md:col-span-2">
                    ...
                    <input v-model="filters.search" type="search" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2" placeholder="Nome, CPF/CNPJ, email" />
                </div>
                <div>
                    <label class="block text-sm font-medium text-slate-700">Tipo</label>
                    <select v-model="filters.tipo_pessoa" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2">
                ...
                <div>
                    ...
                    <select v-model="filters.papel" class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2">
                        <option value="">Todos</option>
                        <option v-for="option in papelOptions" :key="option" :value="option">{{ option }}</option>
                    </select>
                </div>
                <div>
            ...
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Documento</th>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">Papeis</th>
                ...
                <tbody class="divide-y divide-slate-200 text-sm text-slate-700">
                    ...
                    <tr v-else-if="!pessoas.length">
                        <td colspan="6" class="px-4 py-6 text-center text-slate-500">Nenhuma pessoa encontrada.</td>
                    </tr>
                    <tr v-for="pessoa in pessoas" :key="pessoa.id">
                        ...
                        <td class="px-4 py-3">
                            <span v-if="!pessoa.papeis?.length" class="text-slate-400">Sem papeis</span>
                            <div v-else class="flex flex-wrap gap-1">
                                <span v-for="papel in pessoa.papeis" :key="papel" class="inline-flex items-center rounded-full bg-slate-100 px-2 py-0.5 text-xs font-semibold text-slate-700">
                                    {{ papel }}
                                </span>
                            </div>
                        </td>
                        <td class="px-4 py-3 text-right">
                            <Link :href="`/pessoas/${pessoa.id}`" class="text-sm font-semibold text-indigo-600 hover:text-indigo-500">Editar</Link>
                        </td>
                    </tr>
                </tbody>
            </table>
