...
async function submit() {
  ...
  try {
    ...
    if (error?.response?.status === 422) {
      ...
      errorMessage.value = 'Não foi possível salvar o imóvel.';
    }
  } finally {
    saving.value = false;
  }
...
<template>
  <AuthenticatedLayout :title="isEditing ? 'Editar imóvel' : 'Novo imóvel'">
    <div class="mb-6 flex items-center justify-between">
      <h2 class="text-2xl font-semibold text-slate-900">
        {{ isEditing ? 'Editar imóvel' : 'Novo imóvel' }}
      </h2>
      <Link class="text-sm font-semibold text-indigo-600 hover:text-indigo-500" href="/imoveis"
        >Voltar</Link
      >
    </div>
    ...
    <form @submit.prevent="submit" class="grid gap-6 md:grid-cols-2">
      <section class="space-y-4">
        ...
        <div>
          <label class="block text-sm font-medium text-slate-700">Tipo de imóvel</label>
          <input
      ...
      <section class="space-y-4">
        <h3 class="text-sm font-semibold uppercase tracking-wide text-slate-500">Localização</h3>
      ...
      <section class="space-y-4 md:col-span-2">
        ...
        <div class="grid gap-4 md:grid-cols-4">
          <div>
            <label class="block text-sm font-medium text-slate-700">Valor locação</label>
            <input
          ...
          <div>
            <label class="block text-sm font-medium text-slate-700">Valor condomínio</label>
            <input
            ...
            <label class="mt-2 inline-flex items-center gap-2 text-xs text-slate-600">
              <input
                v-model="form.condominio_isento"
                type="checkbox"
                class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500"
              />
              Isento
        ...
        <div class="grid gap-4 md:grid-cols-4">
          <div>
            <label class="block text-sm font-medium text-slate-700">Dormitórios</label>
            <input
          ...
          <div>
            <label class="block text-sm font-medium text-slate-700">Suítes</label>
            <input
        ...
        <div class="grid gap-4 md:grid-cols-2">
          <div>
            <label class="block text-sm font-medium text-slate-700">Área total (m²)</label>
            <input
          ...
          <div>
            <label class="block text-sm font-medium text-slate-700">Área construída (m²)</label>
            <input
        ...
        <div>
          <label class="block text-sm font-medium text-slate-700">Período do IPTU</label>
          <select
        ...
        <div>
          <label class="block text-sm font-medium text-slate-700"
            >Comodidades (separe com vírgula)</label
          >
          <input
            v-model="comodidadesText"
            type="text"
            class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2"
            placeholder="Piscina, Churrasqueira"
          />
        </div>
      </section>
