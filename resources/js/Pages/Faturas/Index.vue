...
function boletoStatusBadgeClass(status: Nullable<string>): string {
  switch (status) {
    case 'paid':
      return 'bg-emerald-500/15 text-emerald-300 border border-emerald-500/40';
    case 'registered':
...
async function copyLinhaDigitavelFromRow(linha: Nullable<string>, faturaId: number) {
  if (!linha) return;

  if (!clipboardSupported) {
    errorMessage.value = 'Copiar linha digitável não é suportado neste dispositivo.';
    return;
  }

  copyingLinhaDigitavelId.value = faturaId;
  ...
  try {
    ...
    copyingLinhaDigitavelId.value = null;
  }
}
        ...
        <table class="min-w-full divide-y divide-slate-800 text-sm">
          <thead class="bg-slate-900/60 text-xs uppercase tracking-wide text-slate-400">
            <tr>
              ...
              <th class="px-4 py-3 text-left">Boleto</th>
          ...
          <tbody class="divide-y divide-slate-800 bg-slate-950/50 text-slate-200">
            <tr v-if="loading">
              <td colspan="10" class="px-4 py-6 text-center text-slate-400">Carregando faturas...</td>
            </tr>
            <tr v-else-if="!hasResults">
              <td colspan="10" class="px-4 py-6 text-center text-slate-400">Nenhuma fatura encontrada.</td>
            </tr>
            <tr v-else v-for="fatura in faturas" :key="fatura.id" class="hover:bg-slate-900/60">
              ...
              <td class="px-4 py-3">
                <template v-if="fatura.boleto_atual">
                  <div class="space-y-1">
                    <span
                      class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold"
                      :class="boletoStatusBadgeClass(fatura.boleto_atual?.status ?? null)"
                    >
                      {{ fatura.boleto_atual?.status_label }}
                    </span>
                    <span v-if="fatura.boleto_atual?.nosso_numero" class="block text-xs text-slate-400">
                      Nosso nº {{ fatura.boleto_atual.nosso_numero }}
                    </span>
                    <div class="flex flex-wrap items-center gap-2 text-xs text-slate-400">
                      <button
                        v-if="fatura.boleto_atual?.linha_digitavel && clipboardSupported"
                        type="button"
                        class="inline-flex items-center gap-1 rounded-md border border-slate-700 px-2 py-1 transition hover:border-indigo-500 hover:bg-indigo-500/20 hover:text-white disabled:cursor-not-allowed disabled:opacity-60"
                        :disabled="copyingLinhaDigitavelId === fatura.id"
                        @click="copyLinhaDigitavelFromRow(fatura.boleto_atual?.linha_digitavel ?? null, fatura.id)"
                      >
                        <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                        ...
                        <span>{{ copyingLinhaDigitavelId === fatura.id ? 'Copiando…' : 'Copiar linha' }}</span>
                      </button>
                      <a
                        v-if="fatura.boleto_atual?.pdf_url"
                        :href="fatura.boleto_atual.pdf_url"
                        target="_blank"
                        rel="noopener noreferrer"
                        class="inline-flex items-center gap-1 rounded-md border border-indigo-500/40 px-2 py-1 text-indigo-200 transition hover:border-indigo-400 hover:bg-indigo-500/20 hover:text-white"
                      >
                        <svg class="h-3.5 w-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
                    ...
                    <span v-if="fatura.boleto_atual?.last_synced_at" class="block text-[11px] text-slate-500">
                      Atualizado {{ formatDateTime(fatura.boleto_atual.last_synced_at) }}
                    </span>
                  </div>
                </template>
                <span v-else class="text-xs text-slate-500">—</span>
