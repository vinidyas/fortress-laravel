...
<template>
  <AuthenticatedLayout :title="isNew ? 'Nova fatura' : `Fatura #${fatura?.id ?? ''}`">
    <div class="mb-6 flex items-center justify-between">
      ...
      <Link class="text-sm font-semibold text-indigo-600 hover:text-indigo-500" href="/faturas"
        >Voltar</Link
      >
    </div>
    ...
    <div
      v-if="loading"
      class="rounded-md border border-slate-200 bg-white px-4 py-6 text-center text-sm text-slate-600"
    >
      Carregando fatura...
    </div>
    ...
    <div v-else>
      ...
      <section class="mb-6 rounded-lg border border-slate-200 bg-white p-6 shadow-sm">
        <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
          <div>
            ...
            <p class="text-lg font-semibold text-slate-900">
              {{ fatura?.contrato?.codigo_contrato ?? '-' }} (ID {{ fatura?.contrato_id }})
            </p>
            <p class="text-sm text-slate-500">
              Imovel: {{ fatura?.contrato?.imovel?.codigo ?? '-' }} /
        ...
        <div class="mt-4 grid gap-4 md:grid-cols-4 text-sm text-slate-700">
          ...
          <div>
            ...
            <p class="font-medium">
              {{ fatura?.valor_pago ? `R$ ${Number(fatura.valor_pago).toFixed(2)}` : '-' }}
            </p>
          </div>
        </div>
      </section>
          ...
          <table class="min-w-full divide-y divide-slate-200">
            ...
            <tbody class="divide-y divide-slate-200 text-sm text-slate-700">
              <tr v-if="!fatura?.itens?.length">
                <td colspan="5" class="px-4 py-6 text-center text-slate-500">Nenhum lançamento.</td>
              </tr>
      ...
      >
        ...
        <form @submit.prevent="settle" class="mt-4 grid gap-4 md:grid-cols-4">
          ...
          <div>
            ...
            <select
              v-model="baixaForm.metodo_pagamento"
              class="mt-1 w-full rounded-md border border-slate-300 px-3 py-2"
            >
      ...
      <section
        v-else-if="fatura?.status !== 'Cancelada'"
        class="rounded-lg border border-slate-200 bg-white p-6 shadow-sm"
      >
        <h3 class="text-sm font-semibold uppercase tracking-wide text-slate-500">Ações</h3>
        <button
          type="button"
          class="mt-3 rounded-md bg-rose-600 px-4 py-2 text-sm font-semibold text-white hover:bg-rose-500"
          @click="cancel"
          :disabled="loading"
        >
          Cancelar fatura
