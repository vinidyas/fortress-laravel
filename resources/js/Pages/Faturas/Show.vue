...
type FaturaBoletoData = {
  id: number;
  fatura_id: number;
  bank_code: string;
  status: string;
  status_label: string;
  valor: Nullable<number | string>;
  valor_pago: Nullable<number | string>;
  vencimento: Nullable<string>;
  registrado_em: Nullable<string>;
  liquidado_em: Nullable<string>;
...
type FaturaData = {
  ...
  boletos?: FaturaBoletoData[];
  boleto_atual?: Nullable<FaturaBoletoData>;
};
...
const boletoAtual = computed<FaturaBoletoData | null>(() => {
  if (fatura.value?.boleto_atual) {
    return fatura.value.boleto_atual;
  }

  return boletos.value.length > 0 ? boletos.value[0] : null;
});
...
async function generateBoleto() {
  ...
  generatingBoleto.value = true;
  errorMessage.value = '';
  successMessage.value = '';

  try {
    const { data } = await axios.post(`/api/faturas/${props.faturaId}/boletos`, {});
    successMessage.value = data?.meta?.message ?? 'Boleto processado com sucesso.';
    await fetchFatura();
  } catch (error: any) {
    console.error(error);
    if (error?.response?.status === 422) {
      ...
      errorMessage.value = error?.response?.data?.message ?? 'Não foi possível gerar o boleto.';
    }
  } finally {
...
async function copyLinhaDigitavel(linha: Nullable<string>) {
  if (!linha) return;
  if (!clipboardSupported) {
    errorMessage.value = 'Seu navegador não suporta cópia automática da linha digitável.';
    return;
  }

  copyingLinhaDigitavel.value = true;

  try {
    await navigator.clipboard.writeText(linha);
    errorMessage.value = '';
    successMessage.value = 'Linha digitável copiada para a área de transferência.';
  } catch (error) {
    console.error(error);
    errorMessage.value = 'Não foi possível copiar a linha digitável.';
  } finally {
    copyingLinhaDigitavel.value = false;
  }
}
...
function boletoStatusBadgeClass(status: string): string {
  switch (status) {
    case 'paid':
...
function formatLinhaDigitavel(value: Nullable<string>): string {
  if (!value) {
    return '—';
  }
      ...
      <div v-else class="space-y-6">
        ...
        <section class="space-y-4 rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-xl shadow-black/40">
          <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
            ...
            <div class="text-right">
              ...
              <div v-if="boletoAtual" class="mt-3 space-y-1 text-right">
                <p class="text-xs uppercase tracking-wide text-slate-500">Boleto</p>
                <div class="inline-flex items-center gap-2">
                  <span
                    class="inline-flex items-center rounded-full px-2.5 py-1 text-xs font-semibold"
                    :class="boletoStatusBadgeClass(boletoAtual.status)"
                  >
                    {{ boletoAtual.status_label }}
                  </span>
                  <span v-if="boletoAtual.last_synced_at" class="text-[11px] text-slate-500">
                    Atualizado {{ formatDateTime(boletoAtual.last_synced_at) }}
                  </span>
                </div>
              </div>
        ...
        <section class="rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-xl shadow-black/40">
          <div class="flex flex-wrap items-start justify-between gap-4">
            <div>
              <h3 class="text-sm font-semibold uppercase tracking-wide text-slate-500">Boleto Bradesco</h3>
              <p class="mt-1 text-xs text-slate-400">Acompanhe emissão, status e dados para cobrança.</p>
            </div>
            <div v-if="boletoAtual" class="text-right text-xs text-slate-400">
              <p v-if="boletoAtual.registrado_em">
                Registrado em
                <span class="font-semibold text-slate-200">{{ formatDateTime(boletoAtual.registrado_em) }}</span>
              </p>
              <p v-if="boletoAtual.liquidado_em" class="text-emerald-200">
                Liquidado em
                <span class="font-semibold">{{ formatDateTime(boletoAtual.liquidado_em) }}</span>
              </p>
            </div>
          </div>

          <div v-if="boletoAtual" class="mt-4 grid gap-4 text-sm text-slate-200 md:grid-cols-3">
            <div>
              <p class="text-xs uppercase tracking-wide text-slate-500">Linha digitável</p>
              <p class="font-semibold text-white break-words">
                {{ boletoLinhaDigitavelFormatada }}
              </p>
            </div>
            <div>
              <p class="text-xs uppercase tracking-wide text-slate-500">Nosso número</p>
              <p class="font-semibold text-white">
                {{ boletoAtual.nosso_numero ?? '—' }}
              </p>
            </div>
            <div>
              <p class="text-xs uppercase tracking-wide text-slate-500">Valor</p>
              <p class="font-semibold text-white">
                {{ boletoAtual.valor ? formatCurrency(boletoAtual.valor) : formatCurrency(fatura?.valor_total ?? '0') }}
              </p>
            </div>
            <div>
              <p class="text-xs uppercase tracking-wide text-slate-500">Vencimento</p>
              <p class="font-semibold text-white">
                {{ formatVencimento(boletoAtual.vencimento) }}
              </p>
            </div>
            <div>
            ...
            <div>
              <p class="text-xs uppercase tracking-wide text-slate-500">Última sincronização</p>
              <p class="font-semibold text-white">
                {{ boletoAtual.last_synced_at ? formatDateTime(boletoAtual.last_synced_at) : '—' }}
              </p>
            </div>
          </div>
          ...
          <p class="mt-4 text-xs leading-relaxed text-slate-400">
            Após gerar o boleto, compartilhe a linha digitável com o locatário ou envie o PDF pelo e-mail integrado. Quando o banco confirmar o pagamento, o status é atualizado automaticamente via webhook.
          </p>
        </section>
