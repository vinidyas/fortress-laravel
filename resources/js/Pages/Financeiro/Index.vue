...
const submitFilters = () => {
    router.get('/financeiro', store.query, {
        preserveState: true,
        preserveScroll: true,
        replace: true,
    });
...
const saldoClasses = computed(() => (props.totals.saldo >= 0 ? 'text-emerald-300' : 'text-rose-300'));
            ...
            <section class="rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-xl shadow-black/40">
                ...
                <div class="mt-6 grid gap-4 md:grid-cols-3">
                    <article class="rounded-xl border border-slate-800 bg-slate-900/70 p-4 text-sm">
                        ...
                        <p class="text-2xl font-semibold text-emerald-300">{{ formatCurrency(props.totals.credito) }}</p>
                    </article>
                    <article class="rounded-xl border border-slate-800 bg-slate-900/70 p-4 text-sm">
                        <p class="text-slate-400">Total débitos</p>
                        <p class="text-2xl font-semibold text-rose-300">{{ formatCurrency(props.totals.debito) }}</p>
                    </article>
                    <article class="rounded-xl border border-slate-800 bg-slate-900/70 p-4 text-sm">
                        <p class="text-slate-400">Saldo</p>
                        <p :class="['text-2xl font-semibold', saldoClasses]">{{ formatCurrency(props.totals.saldo) }}</p>
                    </article>
                </div>
            </section>
