                ...
                <table class="min-w-full divide-y divide-slate-800 text-sm text-slate-100">
                    ...
                    <tbody class="divide-y divide-slate-800">
                        <tr v-if="props.accounts.data.length === 0">
                            <td colspan="4" class="px-4 py-6 text-center text-slate-400">Nenhuma conta cadastrada.</td>
                        </tr>
                        <tr v-for="account in props.accounts.data" :key="account.id">
                            ...
                            <td class="px-4 py-3 text-slate-300">
                                {{ new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(Number(account.saldo_inicial ?? 0)) }}
                            </td>
                            <td class="px-4 py-3">
                                <span
                                    :class="[
                                        'inline-flex rounded-full px-3 py-1 text-xs font-semibold',
                                        account.ativo ? 'bg-emerald-500/10 border border-emerald-500/40 text-emerald-200' : 'bg-slate-700/40 border border-slate-700 text-slate-300',
                                    ]"
                                >
