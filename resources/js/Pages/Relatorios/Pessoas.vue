...
const resumo = ref<{ total: number; por_tipo: Record<string, number>; amostra: Array<Record<string, any>> } | null>(null);
...
<template>
    <AuthenticatedLayout title="Relatorio de pessoas">
        <Head title="Relatorio de pessoas" />
        ...
        <section class="space-y-6 rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-xl shadow-black/40">
            <form class="grid gap-4 md:grid-cols-3" @submit.prevent="loadReport">
                <div>
                    ...
                    <input v-model="filters.papel" type="text" placeholder="ex: Proprietario" class="mt-1 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-white" />
                </div>
                <div>
                    <label class="text-xs font-semibold text-slate-400">Tipo de pessoa</label>
                    <select v-model="filters.tipo_pessoa" class="mt-1 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-white">
            ...
            <div v-if="resumo" class="grid gap-4 md:grid-cols-3">
                ...
                <article v-for="(valor, chave) in resumo.por_tipo" :key="chave" class="rounded-xl border border-slate-800 bg-slate-900/70 p-4 text-sm">
            ...
            <section class="rounded-xl border border-slate-800 bg-slate-900/60">
                <header class="border-b border-slate-800 px-4 py-3 text-sm font-semibold text-white">Amostra</header>
                <div v-if="!resumo || resumo.amostra.length === 0" class="px-4 py-4 text-sm text-slate-400">
                    Nenhum registro encontrado.
                </div>
                <div v-else class="divide-y divide-slate-800">
                    <article v-for="pessoa in resumo.amostra" :key="pessoa.id" class="grid gap-2 px-4 py-4 text-sm text-slate-200 md:grid-cols-4">
                        <p class="font-semibold">{{ pessoa.nome_razao_social }}</p>
                        <p>Tipo {{ pessoa.tipo_pessoa }}</p>
                        <p class="md:col-span-2">Papeis: {{ (pessoa.papeis ?? []).join(', ') || '-' }}</p>
                    </article>
                </div>
            </section>
