...
const ocupacao = ref<{ total: number; disponiveis: number; indisponiveis: number; ocupacao_percentual: number } | null>(null);
...
<template>
    <AuthenticatedLayout title="Relatorio operacional">
        <Head title="Relatorio operacional" />
        ...
        <section class="space-y-6 rounded-2xl border border-slate-800 bg-slate-900/80 p-6 shadow-xl shadow-black/40">
            <form class="grid gap-4 md:grid-cols-4" @submit.prevent="loadReport">
                <div>
                    ...
                    <input v-model="filters.cidade" type="text" class="mt-1 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-white" />
                </div>
                <div>
                    <label class="text-xs font-semibold text-slate-400">Condominio</label>
                    <select v-model="filters.condominio_id" class="mt-1 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-white">
                        <option value="">Todos</option>
                        <option v-for="condominio in props.condominios" :key="condominio.id" :value="condominio.id">
                            {{ condominio.nome }}
                        </option>
                    </select>
                </div>
                <div>
                    ...
                    <select v-model="filters.status_contrato" class="mt-1 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-white">
                ...
                <div>
                    ...
                    <input v-model="filters.ate" type="date" class="mt-1 w-full rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-white" />
                </div>
                <div class="md:col-span-4 flex items-end">
                    <button type="submit" class="w-full rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-500" :disabled="loading">
            ...
            <section class="rounded-xl border border-slate-800 bg-slate-900/60">
                <header class="border-b border-slate-800 px-4 py-3 text-sm font-semibold text-white">Contratos vencendo</header>
                <div v-if="contratos.length === 0" class="px-4 py-4 text-sm text-slate-400">Nenhum contrato proximo do vencimento.</div>
                <div v-for="contrato in contratos" :key="contrato.id" class="flex flex-col gap-2 border-t border-slate-800 px-4 py-4 text-sm text-slate-200 md:flex-row md:items-center md:justify-between">
                    <div>
                        <p class="font-semibold">Contrato {{ contrato.contrato ?? '-' }}</p>
                        <p class="text-xs text-slate-400">Imovel {{ contrato.imovel ?? '-' }} - {{ contrato.cidade ?? '-' }}</p>
                    </div>
            ...
            <div v-if="props.canExport" class="flex gap-3">
                <button type="button" class="rounded-lg border border-slate-700 px-4 py-2 text-sm text-slate-200 hover:bg-slate-800" @click="exportReport">
                    Exportar CSV
                </button>
            </div>
        </section>
