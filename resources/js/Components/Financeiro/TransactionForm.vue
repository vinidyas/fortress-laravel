...
const form = reactive({
    ...
    valor: props.transaction?.valor ? String(props.transaction.valor) : null,
    data_ocorrencia: props.transaction?.data_ocorrencia ?? new Date().toISOString().slice(0, 10),
...
const payloadFromForm = () => {
    ...
    if (form.valor !== null && form.valor !== '') {
        const normalized = typeof form.valor === 'string' ? form.valor : String(form.valor);
        payload.valor = Number.parseFloat(normalized);
    }
...
const handleSubmit = async () => {
    ...
    try {
        ...
        if (error.response?.status === 422) {
            ...
            const message = error.response?.data?.message ?? 'Nao foi possivel salvar o lancamento.';
            toast.error(message);
        }
    } finally {
...
<template>
    <section class="space-y-6">
        <header class="flex flex-col gap-2 border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <div v-if="!canEdit" class="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-700">
                ...
                <div class="grid gap-5 md:grid-cols-2">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <input
                            ...
                            class="rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-indigo-500 focus:outline-none focus:ring-1 focus:ring-indigo-500"
                        />
                        <p v-if="errors.data_ocorrencia" class="text-xs text-rose-600">{{ errors.data_ocorrencia }}</p>
                    </div>
                    <div class="flex flex-col gap-1">
