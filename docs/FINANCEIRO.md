...
- Lançamentos (`financial_transactions`): créditos/débitos opcionais vinculados
  a contratos/faturas.
...
| POST   | `/api/financeiro/transactions/{id}/reconcile` | Concilia lançamento                                               |
...
- `filter[search]`: busca texto em `descricao` e campos textuais de `meta` (JSON
  LIKE).
- `filter[tipo]`: `credito` ou `debito`.
- `filter[status]`: `pendente`, `conciliado`, `cancelado`.
- `filter[account_id]`, `filter[cost_center_id]`, `filter[contrato_id]`,
  `filter[fatura_id]`.
...
3. Executar conciliação via `POST /api/financeiro/transactions/{id}/reconcile`
   informando `valor_conciliado` e observação.
4. Para lançamentos incorretos, use
   `POST /api/financeiro/transactions/{id}/cancel` (bloqueado se já conciliado).

### Exportação

- `GET /api/financeiro/transactions/export` gera CSV com filtros aplicados.
